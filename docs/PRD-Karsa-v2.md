...
**Definisi role (dipertegas Fase 4C):** Admin boleh merangkap semua role, termasuk PJ dan mahasiswa. PJ = user dengan **≥1 `KelasMatkul.pj_id = dirinya`**, tanpa mempedulikan `is_admin`. Konsekuensi teknis:
- `is_pj` dihitung dari relasi `KelasMatkul` (`lib/user-snapshot.ts`), bukan kolom di `User`, dan tidak dibatalkan oleh `is_admin`.
- `actions/mahasiswa.ts` menerima admin sebagai anggota kelas selama domain email, kelas tunggal, dan pemeriksaan duplikasi terpenuhi. `resolvePj()` tetap menerima admin sebagai PJ dan mengecualikannya dari syarat "PJ harus anggota kelas", sehingga admin dapat pula memegang penugasan di kelas lain. Kaitan PJ↔kelas tetap ada di `KelasMatkul.kelas_id`, dan `createPoinLog()` mengambil mahasiswa dari `kelas_matkul.kelas_id` (bukan dari `kelas_id` milik PJ).
...
| **Fase 6A-2 — Audit trail CRUD admin** | `SELESAI` | Mutasi mahasiswa, kelas, matkul, prodi, dan semester mencatat event `AuditLog` dengan snapshot before/after yang relevan. Mutasi domain dan audit ditulis atomik dalam satu `prisma.$transaction`. |
| **Fase 3A — Input poin PJ (mobile)** | `SELESAI` | Diverifikasi end-to-end oleh user pada 2026-09-17: input 4 poin kategori Bertanya untuk Agus Santoso dari HP di `/catat-poin` berhasil, toast konfirmasi tampil, dan baris `PoinLog` terverifikasi di Supabase (`id` `r2000i7f0439guvcj9`). `actions/poin.ts` (`getMatkulsAsPj`, `getMahasiswaInKelasMatkul`, `getKategoriPoin`, `createPoinLog`) — semua ber-guard `requireUser()` + re-query `KelasMatkul.pj_id === session.user.id` dari DB (anti-IDOR), mahasiswa wajib memiliki `kelas_id` penugasan (termasuk bila ia juga admin), PJ tidak bisa menilai diri sendiri, `poin` integer 1–4 (Zod + CHECK DB), catatan ≤500 char, anti double-submit ≤3 detik, `pj_id` selalu dari session. UI: `/catat-poin` (kartu matkul + jumlah mahasiswa) → `/catat-poin/[kelas_matkul_id]` (list mahasiswa, `notFound()` bila bukan PJ-nya) → bottom sheet `vaul` (segmented kategori, tombol poin 1–4, catatan collapsible, haptic 10ms + toast Sonner + `router.refresh()`). Aturan role: **admin boleh merangkap PJ dan mahasiswa** (`actions/mahasiswa.ts`, `actions/kelas-matkul.ts`, dropdown PJ di tab Matkul & PJ). `tsc` · `lint` · `build` bersih di sandbox. |
